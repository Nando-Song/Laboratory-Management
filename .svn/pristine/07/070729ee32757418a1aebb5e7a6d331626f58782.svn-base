...
namespace LABMANAGE.Controllers
{
    public class InfoController : Controller
    {
        ...
        public ActionResult modifyInfo(string name, string password, string motto, string old_password, string Email, string Phone)
        {
            int id, seid;
            seid = Convert.ToInt32(LoginBase.ID);
            if (seid == 0)
            ...
            //}
userService.UpdateMessage(name, motto, Email, Phone);
            List<UserBaseDto> list = new List<UserBaseDto>();
            list = userService.GetAll(id);
